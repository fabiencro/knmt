...
class StackedCell(ChainList):
    def __init__(self, in_size, out_size, cell_type = LSTMCell, nb_stacks = 2, dropout = 0.5, init = None, inner_init = None, lateral_init = None, upward_init = None, bias_init = None, forget_bias_init = None):
...
def create_cell_model_from_string(model_str):
    ...
    for comp in components[1:]:
        ...
        elif key.endswith("init_type"):
            keywords[key] = val
        elif key.endswith("init_scale"):
            keywords[key] = float(val)
        elif key.endswith("init_fillvalue"):
            keywords[key] = float(val)
        else:
...
def create_cell_model(type_str, dropout = None, nb_stacks = None, sub_cell_type = None, **keywords):
    initializers = create_initializer_table(keywords)
