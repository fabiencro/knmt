...
#         out.write(convert_idx_to_string(t, tgt_voc) + "\n")
    ...
    if test_references is not None:
        ...
        bc = compute_bleu_with_unk_as_wrong(test_references,
                                            [t[:-1] for t in translations],
                                            tgt_indexer.is_unk_idx,
                                            new_unk_id_ref,
                                            new_unk_id_cand)
        log.info("bleu: %r" % bc)
        return bc
...
def compute_loss_all(
        ...
        reverse_src=False,
        reverse_tgt=False):
    mb_provider_test = minibatch_provider(
        test_data,
        eos_idx,
        mb_size,
        nb_mb_for_sorting=-1,
...
def greedy_batch_translate(
    ...
    for i in range(nb_batch):
        ...
        src_batch, src_mask = make_batch_src(
            current_batch_raw_data, gpu=gpu, volatile="on")
        sample_greedy, score, attn_list = encdec(
            src_batch, nb_steps, src_mask, use_best_for_sample=True, keep_attn_values=get_attention, mode="test")
        deb = de_batch(
            sample_greedy,
            mask=None,
            eos_idx=eos_idx,
...
def reverse_rescore(
    ...
    tgt_batch, arg_sort = utils.make_batch_tgt(
        reversed_translations, eos_idx=eos_idx, gpu=gpu, volatile="on", need_arg_sort=True)
...
#     res = []
    for num_ex in range(nb_ex):
        src_batch, src_mask = make_batch_src(
            [src_data[num_ex]], gpu=gpu, volatile="on")
...
#                                     groundhog = groundhog)
        ...
        # TODO: This is a quick patch, but actually ensemble_beam_search
        # probably should not return empty translations except when no
        # translation found
        ...
        if post_score_length_normalization == 'none' and post_score_coverage_penalty == 'none':
            ...
            def ranking_criterion(x):
                ...
                if post_score_length_normalization == 'simple':
                    ...
                    length_normalization = pow((len(x[0]) + 5),
                                               post_score_length_normalization_strength) / pow(6,
...
def batch_align(encdec, eos_idx, src_tgt_data, batch_size=80, gpu=None):
    ...
    attn_all = []
    for i in range(nb_batch):
        current_batch_raw_data = src_tgt_data[i *
...
#         print current_batch_raw_data
        ...
        loss, attn_list = encdec(
            src_batch, tgt_batch, src_mask, keep_attn_values=True)
        deb_attn = de_batch(
            attn_list,
            mask=None,
            eos_idx=None,
...
def sample_once(
    ...
    print "sample"
    sample_greedy, score, attn_list = encdec(
        src_batch, 50, src_mask, use_best_for_sample=True, need_score=True, mode="test")
...
#                 sample, score = encdec(src_batch, 50, src_mask, use_best_for_sample = False)
    ...
    sample_random, score_random, attn_list_random = encdec(
        src_batch, 50, src_mask, use_best_for_sample=False, need_score=True, mode="test")
