...
def define_parser(parser):
    parser.add_argument(
        "training_config",
        nargs="?",
        help="prefix of the trained model",
    ...
    translation_method_group = parser.add_argument_group(
        _CONFIG_SECTION_TO_DESCRIPTION["method"])
    translation_method_group.add_argument(
        "--mode",
        default="translate",
        choices=[
            "translate",
        ...
        help="target text")
    translation_method_group.add_argument(
        "--beam_width", type=int, default=20, help="beam width")
    translation_method_group.add_argument(
        "--beam_pruning_margin",
        type=float,
        default=None,
        help="beam pruning margin")
    parser.add_argument(
    ...
    translation_method_group.add_argument(
        ...
        type=int,
        default=50,
        help="nb_steps used in generation")
    translation_method_group.add_argument(
        "--nb_steps_ratio",
        type=float,
        help="nb_steps used in generation as a ratio of input length")
#     translation_method_group.add_argument("--beam_opt", default = False, action = "store_true")
    translation_method_group.add_argument(
        "--groundhog", default=False, action="store_true")
    translation_method_group.add_argument(
        "--force_finish", default=False, action="store_true")
    translation_method_group.add_argument(
    ...
    translation_method_group.add_argument(
        ...
        default='none')
    translation_method_group.add_argument(
        "--post_score_coverage_penalty_strength", type=float, default=0.2)
    translation_method_group.add_argument(
    ...
    output_group = parser.add_argument_group(
        _CONFIG_SECTION_TO_DESCRIPTION["output"])
    output_group.add_argument("--tgt_fn", help="target text")
    output_group.add_argument(
        "--nbest_to_rescore",
        help="nbest list in moses format")
    ...
    output_group.add_argument(
        ...
        help="generate a html file with attention information")
    output_group.add_argument(
        "--rich_output_filename",
        help="generate a JSON file with attention information")
    ...
    management_group = parser.add_argument_group(
        _CONFIG_SECTION_TO_DESCRIPTION["process"])
    ...
    management_group.add_argument(
        "--load_model_config",
        nargs="+",
        help="gives a list of models to be used for translation")
    management_group.add_argument(
        "--src_fn",
        nargs="?",
        help="source text",
    ...
    management_group.add_argument(
        ...
        help="prefix of the trained model")
    management_group.add_argument(
        "--additional_trained_model",
        nargs="*",
        help="prefix of the trained model")
    management_group.add_argument(
        "--reverse_training_config",
        help="prefix of the trained model")
    management_group.add_argument(
        "--reverse_trained_model",
        help="prefix of the trained model")
#     management_group.add_argument("--config", help = "load eval config file")
    management_group.add_argument(
        "--server", help="host:port for listening request")
    management_group.add_argument(
        "--segmenter_command",
        help="command to communicate with the segmenter server")
    management_group.add_argument(
        "--segmenter_format",
        help="format to expect from the segmenter (parse_server, morph)",
        default='plain')
    management_group.add_argument(
        "--description",
    ...
    if config_eval.process.server is None:
        if config_eval.process.src_fn is None or config_eval.process.dest_fn is None:
            raise CommandLineValuesException(
                "src_fn and dest_fn need to be set either on the command line or in a config file")
...
def load_config_eval(filename, readonly=True):
    ...
    if "metadata" not in config:  # older config file
        parse_option_orderer = get_parse_option_orderer()
        config_eval = parse_option_orderer.convert_args_to_ordered_dict(
