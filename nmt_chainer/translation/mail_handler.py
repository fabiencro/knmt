    ...
    if (len(id_list) > 0):
        ...
        for mail_id in range(first_email_id, latest_email_id + 1):
            ...
            try:
                ...
                for response_part in data:
                    if isinstance(response_part, tuple):
                        ...
                        if subject_match:
                            ...
                            logger.info("Id: {0}".format(mail_id))
                            logger.info("Date: {0}".format(email_date))
                            logger.info("From: {0}".format(email_from))
                            logger.info("Subject: {0}".format(email_subject))

                            requests.append(MailRequest(mail_id, email_date, email_from, email_subject, email_body))

                            reply = "Translation request received.\n\n{0}\n\nPlease wait...\n\n---------- Original text submitted on {1} ----------\n\nLanguage pair: {2}-{3}\n\n{4}\n\n-- The content of this message has been generated by KNMT {5}. --"
                            if (len(requests) <= 1):
                                queue_msg = "It will be processed shortly."
                            else:
                ...
                logger.info(reply)
                mail.copy(mail_id, config['imap']['ignored_request_mailbox'])
        ...
        try:
            ...
            for req in requests:
                try:
                    ...
                    logger.info("Id: {0}".format(req.id))
                    ...
                    mail.copy(req.id, config['imap']['processed_request_mailbox'])
                    mail.store(req.id, '+FLAGS', '\\Deleted')
                    mail.expunge()

                except Exception, ex_msg:
                    logger.info('Error: {0}'.format(ex_msg))
                    logger.info("Moving message to Ignored mailbox.\n\n")
                    mail.copy(req.id, config['imap']['ignored_request_mailbox'])
                    mail.store(req.id, '+FLAGS', '\\Deleted')
                    mail.expunge()
