...
def beam_search_all(
    ...
    log.info(
        "starting beam search translation of %i sentences" %
        len(src_data))
...
#                 ct = convert_idx_to_string_with_attn(t, tgt_voc, attn, unk_idx = len(tgt_voc))
            ...
            if ct != '':
                ...
                if replace_unk:
                    ...
                    translated = replace_tgt_unk.replace_unk_from_string(
                        ct,
                        src,
                        dic,
                        remove_unk,
                        normalize_unicode_unk,
                        attempt_to_relocate_unk_source).strip().split(" ")

            yield src_data[num_t], translated, t, score, attn, unk_mapping
        print >>sys.stderr


def translate_to_file_with_beam_search(
        dest_fn,
        gpu,
        encdec,
        eos_idx,
        ...
        attn_graph_with_sum=True,
        attn_graph_attribs=None,
        src_indexer=None,
    ...
    translation_iterator = beam_search_all(
        gpu,
        encdec,
        eos_idx,
        src_data,
    ...
    for src, translated, t, score, attn, unk_mapping in translation_iterator:
        if rich_output is not None:
            rich_output.add_info(
                src,
                translated,
                t,
                score,
                attn,
                unk_mapping=unk_mapping)
...
def create_and_load_encdec_from_files(config_training_fn, trained_model):
    ...
    encdec, eos_idx, src_indexer, tgt_indexer = train.create_encdec_and_indexers_from_config_dict(
        config_training)
...
def check_if_vocabulary_info_compatible(
        this_eos_idx,
        this_src_indexer,
        this_tgt_indexer,
        eos_idx,
...
def create_encdec(config_eval):
    ...
    if 'load_model_config' in config_eval.process and config_eval.process.load_model_config is not None:
        for config_filename in config_eval.process.load_model_config:
            ...
            encdec, this_eos_idx, this_src_indexer, this_tgt_indexer = train.create_encdec_and_indexers_from_config_dict(
                config_training, load_config_model="yes")
            ...
            if eos_idx is None:
                ...
                check_if_vocabulary_info_compatible(
                    this_eos_idx,
                    this_src_indexer,
                    this_tgt_indexer,
                    eos_idx,
    ...
    if 'additional_training_config' in config_eval.process and config_eval.process.additional_training_config is not None:
        assert len(
            config_eval.process.additional_training_config) == len(
            config_eval.process.additional_trained_model)

        for (
                config_training_fn,
                trained_model_fn) in zip(
...
#                 this_encdec = this_encdec.to_gpu(args.gpu)
    ...
    if 'gpu' in config_eval.process and config_eval.process.gpu is not None:
        encdec_list = [encdec.to_gpu(config_eval.process.gpu)
...
#     json.dump(config_eval, open(save_eval_config_fn, "w"), indent=2, separators=(',', ': '))
    ...
    encdec_list, eos_idx, src_indexer, tgt_indexer, reverse_encdec = create_encdec(
        config_eval)

    if src_fn is not None:
        log.info("opening source file %s" % src_fn)
        src_data, stats_src_pp = build_dataset_one_side_pp(
            src_fn, src_pp=src_indexer, max_nb_ex=max_nb_ex)
...
#                                                                     make_data_infos.total_token))
    ...
    if tgt_fn is not None:
        ...
        tgt_data, stats_tgt_pp = build_dataset_one_side_pp(
            tgt_fn, src_pp=tgt_indexer, max_nb_ex=max_nb_ex)
...
#             assert len(attn) == len(tgt_idx_list)
            ...
            tgt_w = tgt_indexer.deconvert_swallow(
                tgt_idx_list, unk_tag="#T_UNK#")
    ...
    # print [src_voc_with_unk[idx] for idx in src_idx_list], tgt_idx_list
        ...
        with cuda.get_device(gpu):
            ...
            loss, attn_all = batch_align(
                encdec_list[0], eos_idx, zip(
                    src_data, tgt_data), batch_size=mb_size, gpu=gpu)
#         tgt_voc_with_unk = tgt_voc + ["#T_UNK#"]
#         src_voc_with_unk = src_voc + ["#S_UNK#"]
...
# out.write(convert_idx_to_string([x for x, attn in t], tgt_voc +
# ["#T_UNK#"]) + "\n")
        ...
        log.info(
            "#tokens src: %i   of which %i (%f%%) are unknown" %
            (make_data_infos.total_token,
             make_data_infos.total_count_unk,
             float(
                 make_data_infos.total_count_unk *
        ...
        for num in xrange(len(nbest_converted)):
            ...
            src_batch, src_mask = utils.make_batch_src(
                [src], gpu=gpu, volatile="on")
            ...
            for num_batch in xrange(nb_batches):
                tgt_batch, arg_sort = utils.make_batch_tgt(tgt_list[num_batch * nb_batches: (
                    num_batch + 1) * nb_batches], eos_idx=eos_idx, gpu=gpu, volatile="on", need_arg_sort=True)
