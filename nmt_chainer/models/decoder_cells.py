...
class ConditionalizedDecoderCell(object):
    def __init__(
        ...
        if noise_on_prev_word:
            self.noise_mean = self.xp.ones(
                (mb_size, self.decoder_chain.Eo), dtype=self.xp.float32)
            self.noise_lnvar = self.xp.zeros(
                (mb_size, self.decoder_chain.Eo), dtype=self.xp.float32)
    ...
    def advance_state(self, previous_states, prev_y):
        ...
        new_states = self.decoder_chain.gru(
            previous_states, concatenated, mode=self.mode)
    ...
    def compute_logits(self, new_states, concatenated, attn):
        ...
        logits = self.decoder_chain.lin_o(
            self.decoder_chain.maxo(all_concatenated))
        ...
        if self.lexicon_probability_matrix is not None:
            ...
            if self.demux:
                ...
                weighted_lex_probs = F.reshape(
                    matmul_constant(
                        attn,
                        lexicon_probability_matrix.reshape(
                            lexicon_probability_matrix.shape[1],
    ...
    #                                                logits.data.shape)
                ...
                weighted_lex_probs = F.reshape(
                    batch_matmul_constant(
                        attn,
                        lexicon_probability_matrix,
                        transa=True),
                    logits.data.shape)
    ...
    def advance_one_step(self, previous_states, prev_y):
        ...
        new_states, concatenated, attn = self.advance_state(
    ...
    def get_initial_logits(self, mb_size=None):
        ...
        prev_y = F.broadcast_to(
            self.decoder_chain.bos_embeding, (mb_size, self.decoder_chain.Eo))
...
def compute_loss_from_decoder_cell(
        cell,
        targets,
...
#         print "loss this step", loss.data if loss is not None else None, total_local_loss.data
        ...
        if i >= len(targets) - \
                1:  # skipping generation of last states as unneccessary
            break
        ...
        if use_previous_prediction > 0 and random.random() < use_previous_prediction:
            previous_word = Variable(cell.xp.argmax(
                logits.data[:required_next_mb_size], axis=1).astype(cell.xp.int32), volatile="auto")
        else:
            if required_next_mb_size < current_mb_size:
                previous_word, _ = F.split_axis(
                    targets[i], (required_next_mb_size,), 0)
...
#                     curr_idx[i] =  sampler.sample(1)[0]
        if need_score:
            score = score + np.log(cuda.to_cpu(probs.data)
                                   [np.arange(cell.mb_size), cuda.to_cpu(curr_idx)])
...
#         self.initial_state.data[...] = np.random.randn(Ho)
    ...
    def give_conditionalized_cell(
        ...
        if not demux:
            return ConditionalizedDecoderCell(
                self,
                compute_ctxt,
                mb_size,
            ...
            assert mb_size == 1
            assert demux >= 1
            compute_ctxt = self.attn_module.compute_ctxt_demux(
                fb_concat, src_mask)
            return ConditionalizedDecoderCell(
                self,
                compute_ctxt,
    ...
    def compute_loss(
            ...
            lexicon_probability_matrix=None,
            lex_epsilon=1e-3):
        decoding_cell = self.give_conditionalized_cell(
            fb_concat,
            src_mask,
            noise_on_prev_word=noise_on_prev_word,
            mode=mode,
    ...
    def sample(
        ...
        decoding_cell = self.give_conditionalized_cell(
            ...
            lexicon_probability_matrix=lexicon_probability_matrix,
            lex_epsilon=lex_epsilon)
        sequences, score, attn_list = sample_from_decoder_cell(
            decoding_cell, nb_steps, best=best, keep_attn_values=keep_attn_values, need_score=need_score)
