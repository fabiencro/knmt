...
#         tgt_pp = IndexingPrePostProcessor.make_from_serializable(tgt_voc)
        ...
        if config.source_char_conversion is not None:
            log.info("using source char conversion %s", config.source_char_conversion)
            char_conv_dic = json.load(open(config.source_char_conversion))
