...
_CONFIG_SECTION_TO_DESCRIPTION = {
    "data": "Files used for training, dev, testing, ...",
...
def define_parser(parser):
    parser.add_argument(
        ...
        help="source language text file for training data")
    parser.add_argument(
        "tgt_fn",
        nargs="?",
        action=argument_parsing_tools.ArgumentActionNotOverwriteWithNone,
        help="target language text file for training data")
    parser.add_argument(
        "save_prefix",
        nargs="?",
        action=argument_parsing_tools.ArgumentActionNotOverwriteWithNone,
        help="created files will be saved with this prefix")
    ...
    data_group = parser.add_argument_group(
        _CONFIG_SECTION_TO_DESCRIPTION["data"])
    ...
    processing_group = parser.add_argument_group(
        _CONFIG_SECTION_TO_DESCRIPTION["processing"])
    ...
    processing_group.add_argument("--use_voc",
                                  help="specify an exisiting vocabulary file")
...
#     parser.add_argument("--enable_fast_shuffle", default = False, action = "store_true")
    ...
    processing_group.add_argument(
        "--bpe_src",
        type=int,
        help="Apply BPE to source side, with this many merges")
    processing_group.add_argument(
        "--bpe_tgt",
        type=int,
        help="Apply BPE to target side, with this many merges")

    processing_group.add_argument(
        "--joint_bpe",
        type=int,
        help="Apply joint BPE, with this many merges")

    processing_group.add_argument(
        "--latin_src",
        default=False,
        action="store_true",
        help="apply preprocessing for latin scripts to source")
    processing_group.add_argument(
        "--latin_tgt",
        default=False,
        action="store_true",
        help="apply preprocessing for latin scripts to target")

    processing_group.add_argument(
        "--latin_type",
        choices="all_adjoint caps_isolate".split(),
        default="all_adjoint",
        help="choose preprocessing for latin scripts to source")
...
def make_data_config(args):
    ...
    if config["data"]["src_fn"] is None or config["data"]["tgt_fn"] is None or config["data"]["save_prefix"] is None:
        raise CommandLineValuesException(
            "src_fn, tgt_fn and save_prefix need to be set either on the command line or in a config file")
    ...
    if not ((config.data.dev_src is None) == (config.data.dev_tgt is None)):
        raise CommandLineValuesException(
            "Command Line Error: either specify both --dev_src and --dev_tgt or neither")

    if config.processing.joint_bpe is not None and (
            config.processing.bpe_src is not None or config.processing.bpe_tgt is not None):
        raise CommandLineValuesException(
            "Command Line Error: --joint_bpe is incompatible with --bpe_src and --bpe_tgt")
    return config
...
def load_config(filename):
    ...
    if "metadata" not in config:  # older config file
        parse_option_orderer = get_parse_option_orderer()
        config = parse_option_orderer.convert_args_to_ordered_dict(
            config, args_is_namespace=False)
    config.set_readonly()
    return config
