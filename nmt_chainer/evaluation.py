...
def beam_search_translate(encdec, eos_idx, src_data, beam_width = 20, beam_pruning_margin = None, nb_steps = 50, gpu = None, beam_opt = False,
                          need_attention = False, nb_steps_ratio = None, post_score_length_normalization = 'simple', post_score_length_normalization_strength = 0.2,  
...
#         bests.append(translations[0])
        ...
        if post_score_length_normalization == 'simple':
            ...
            translations.sort(key = lambda x:x[1]/( pow((len(x[0])+5), post_score_length_normalization_strength) / pow(6, post_score_length_normalization_strength) ), reverse = True)
        else:
