...
_CONFIG_SECTION_TO_DESCRIPTION = {
...
def define_parser(parser):
    ...
    model_description_group = parser.add_argument_group(
        _CONFIG_SECTION_TO_DESCRIPTION["model"])
    model_description_group.add_argument(
        "--Ei", type=int, default=640, help="Source words embedding size.")
    model_description_group.add_argument(
        "--Eo", type=int, default=640, help="Target words embedding size.")
    model_description_group.add_argument(
        "--Hi",
        type=int,
        default=1024,
    ...
    model_description_group.add_argument(
        "--Ho", type=int, default=1024, help="Target hidden layer size.")
    model_description_group.add_argument(
        "--Ha",
        type=int,
        default=1024,
    ...
    model_description_group.add_argument(
        "--Hl", type=int, default=512, help="Maxout output size.")
    model_description_group.add_argument(
        "--encoder_cell_type",
        default="lstm",
        help="cell type of encoder. format: type,param1:val1,param2:val2,...")  # where type is in [%s]"%(" ".join(rnn_cells.cell_dict.keys())))
    model_description_group.add_argument(
        "--decoder_cell_type",
        default="lstm",
    ...
    model_description_group.add_argument(
        ...
        help="lexical translation probabilities in zipped JSON format. Used to implement https://arxiv.org/abs/1606.02006")
    model_description_group.add_argument(
    ...
    model_description_group.add_argument(
        "--use_deep_attn", default=False, action="store_true")
    model_description_group.add_argument(
        "--use_accumulated_attn",
        default=False,
        action="store_true")
    model_description_group.add_argument(
        "--init_orth", default=False, action="store_true")
    model_description_group.add_argument(
        "--use_bn_length", default=0, type=int)

    training_paramenters_group = parser.add_argument_group(
        _CONFIG_SECTION_TO_DESCRIPTION["training"])
    training_paramenters_group.add_argument(
        "--mb_size", type=int, default=64, help="Minibatch size")
    training_paramenters_group.add_argument(
        "--nb_batch_to_sort",
        type=int,
        default=20,
    ...
    training_paramenters_group.add_argument(
        "--noise_on_prev_word", default=False, action="store_true")
    training_paramenters_group.add_argument(
    ...
    training_paramenters_group.add_argument(
        "--hard_gradient_clipping",
        type=float,
        nargs=2,
        help="hard gradient clipping.")
    training_paramenters_group.add_argument(
        "--weight_decay", type=float, help="Weight decay value. ")
    training_paramenters_group.add_argument(
        "--optimizer",
        choices=[
            "sgd",
    ...
    training_paramenters_group.add_argument(
        ...
        help="Learning Rate")
    training_paramenters_group.add_argument(
        "--momentum", type=float, default=0.9, help="Momentum term")
    training_paramenters_group.add_argument(
        "--randomized_data", default=False, action="store_true")
    training_paramenters_group.add_argument(
        "--no_shuffle_of_training_data",
        default=False,
        action="store_true")
    training_paramenters_group.add_argument(
        "--use_reinf", default=False, action="store_true")
    training_paramenters_group.add_argument(
        "--use_previous_prediction", default=0, type=float)
    training_paramenters_group.add_argument(
        "--curiculum_training", default=False, action="store_true")
    training_paramenters_group.add_argument(
        "--reverse_src", default=False, action="store_true")
    training_paramenters_group.add_argument(
        "--reverse_tgt", default=False, action="store_true")

    training_monitoring_group = parser.add_argument_group(
        _CONFIG_SECTION_TO_DESCRIPTION["training_management"])
    training_monitoring_group.add_argument(
        "--config", help="load a training config file")
    training_monitoring_group.add_argument(
        "--data_prefix",
        dest="data_prefix",
        action=argument_parsing_tools.ArgumentActionNotOverwriteWithNone,
    ...
    training_monitoring_group.add_argument(
        "--gpu", type=int, help="specify gpu number to use, if any")
    training_monitoring_group.add_argument(
        "--load_model", help="load the parameters of a previously trained model")
    training_monitoring_group.add_argument(
        "--load_optimizer_state",
        help="load previously saved optimizer states")
    training_monitoring_group.add_argument(
        "--load_trainer_snapshot",
        help="load previously saved trainer states")
    training_monitoring_group.add_argument(
    ...
    training_monitoring_group.add_argument(
        "--no_resume", default=False, action="store_true")
    training_monitoring_group.add_argument(
        "--no_report_or_save", default=False, action="store_true")
    training_monitoring_group.add_argument(
        "--sample_every", default=200, type=int)
    training_monitoring_group.add_argument(
        "--save_ckpt_every", default=4000, type=int)
    training_monitoring_group.add_argument(
        "--save_initial_model_to",
        help="save the initial model parameters to given file in npz format")
    training_monitoring_group.add_argument(
        "--reshuffle_every_epoch",
        default=False,
        action="store_true",
        help="reshuffle training data at the end of each epoch")
    training_monitoring_group.add_argument(
        "--resume",
        default=False,
        action="store_true",
        help="resume training from checkpoint config")
    training_monitoring_group.add_argument(
        "--timer_hook",
        default=False,
        action="store_true",
        help="activate timer hook for profiling")
    training_monitoring_group.add_argument(
        "--force_overwrite",
        default=False,
        action="store_true",
        help="Do not ask before overwiting existing files")
    training_monitoring_group.add_argument(
        "--description",
...
def get_parse_option_orderer():
    description_to_config_section = dict((v, k) for (
        k, v) in _CONFIG_SECTION_TO_DESCRIPTION.iteritems())
    por = argument_parsing_tools.ParseOptionRecorder(
        group_title_to_section=description_to_config_section,
...
def convert_cell_string(config_training, no_error=False):
    ...
    try:
        if "encoder_cell_type" in config_training["model"] and config_training[
                "model"]["encoder_cell_type"] is not None:
        ...
        if "decoder_cell_type" in config_training["model"] and config_training[
                "model"]["decoder_cell_type"] is not None:
...
def make_config_from_args(args, readonly=True):
    ...
    if config_base is not None:
        ...
        config_base.update_recursive(
            config_training, valid_keys=args_given_set)
...
#     config_data_fn = config_training["data_prefix"] + ".data.config"
    ...
    if config_training["training_management"]["data_prefix"] is None or config_training["training_management"]["save_prefix"] is None:
        raise CommandLineValuesException(
            "save_prefix and data_prefix need to be set either on the command line or in a config file")

    config_training.add_metadata_infos(
        version_num=1, overwrite=args.config is not None)
