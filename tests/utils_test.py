...
class TestDeBatch:
    def test_multiple_length(self):
        batch = [np.array([1, 3, 4, 8]), np.array(
            [1, 5, 6, 9]), np.array([7, 5]), np.array([10])]
        seq_list = de_batch(batch)

        assert seq_list == [[1, 1, 7, 10], [3, 5, 5], [4, 6], [8, 9]]

    def test_multiple_length_variable(self):
        batch = [Variable(np.array(x, dtype=np.int32))
    ...
    def test_multiple_length_variable_raw(self):
        batch = [Variable(np.array(x, dtype=np.int32)) for x in [
            [1, 3, 4, 8], [1, 5, 6, 9], [[7, 9], [5, 8]], [10]]]
        seq_list = de_batch(batch, is_variable=True, raw=True)
        assert len(seq_list) == 4
        for seq1, seq2 in zip(seq_list, [[1, 1, [7, 9], 10], [
                              3, 5, [5, 8]], [4, 6], [8, 9]]):
    ...
    def test_multiple_length_eos_idx(self):
        batch = [np.array([1, 3, 4, 8]), np.array(
            [3, 3, 6, 9]), np.array([7, 5]), np.array([10])]
        seq_list = de_batch(batch, eos_idx=3)
        assert seq_list == [[1, 3], [3], [4, 6], [8, 9]]

    def test_mask1(self):
        batch = [np.array([1, 3, 4, 8]), np.array(
            [1, 5, 6, 9]), np.array([7, 5, 3, 4])]
        mask = [np.array([True, True, True, True]), np.array(
            [True, True, True, True]), np.array([True, True, True, True])]
    ...
    def test_mask2(self):
        batch = [np.array([1, 3, 4, 8]), np.array(
            [1, 5, 6, 9]), np.array([7, 5, 3, 4])]
        mask = [np.array([True, True, True, True]), np.array(
            [True, True, False, True]), np.array([True, True, False, False])]
        seq_list = de_batch(batch, mask=mask)
...
class TestMakeData:
    def test_data_creation(self, tmpdir, gpu):
        ...
        args = [
            "make_data", os.path.join(
                test_data_dir, "src2.txt"), os.path.join(
                test_data_dir, "tgt2.txt"), data_prefix]
        main(arguments=args)
