...
# $Id: SentenceExtractor.pm,v 1.12 2013/06/20 14:20:04 kawahara Exp $
...
@honorifics = qw(Adj. Adm. Adv. Asst. Bart. Brig. Bros. Capt. Cmdr. Col. Comdr. Con. Cpl. Dr. Ens. Gen. Gov. Hon. Hosp. Insp. Lt. M. MM. Maj. Messrs. Mlle. Mme. Mr. Mrs. Ms. Msgr. Op. Ord. Pfc. Ph. Prof. Pvt. Rep. Reps. Res. Rev. Rt. Sen. Sens. Sfc. Sgt. Sr. St. Supt. Surg. vs. v. i.e. e.g. L.A. U.S. etc.);
...
sub FixParenthesis {
    ...
    for my $i (0 .. scalar(@{$slist} - 1)) {
        ...
        #
        # I believe that this case applies for a sentence that uses only the ” mark
        # for both opening and closing quotation. However, there are cases where
        # both “ and ” are used for opening and closing a quotation, respectively.
        # To deal with such sentences, I subtract the count of closing marks to
        # cancel the count of opening marks. - FB
        if ($i > 0) {
            my $num_of_zenaku_quote_prev = scalar(split('”', $slist->[$i - 1], -1)) - 1;
            my $num_of_zenaku_quote_curr = scalar(split('”', $slist->[$i], -1)) - 1;
            my $num_of_closing_zenaku_quote_prev = scalar(split('“', $slist->[$i - 1], -1)) - 1;
            my $num_of_closing_zenaku_quote_curr = scalar(split('“', $slist->[$i], -1)) - 1;
            
            if ($num_of_closing_zenaku_quote_prev > 0) {
                $num_of_zenaku_quote_prev -= $num_of_closing_zenaku_quote_prev;
            }
            if ($num_of_closing_zenaku_quote_curr > 0) {
                $num_of_zenaku_quote_curr -= $num_of_closing_zenaku_quote_curr;
            }
...
sub SplitJapanese {
    ...
    while ($str =~ /(?:(?:$period)|((?:$cdot){3,})|(?<!$alphabet_or_number)(?:$dot)(?!$alphabet_or_number|$comma))(?:$dot|$period)*/o) { # check delimiter & split
        ...
        if ($1 eq '' || $pre ne '') { # a sentence should include strings other than $cdot{3,}
            ...
            #     I disable the test because it seems to cause some problems (see Issue #17). - FB
            # if ($level == 0) {
                push (@tmp, $sent);
                $sent = '';
            # }
...
sub SplitEnglish
{
    ...
    for $i (0..$#words)
    {
        ...
        if ($newword =~ /\([^)]*$/) {
            # print STDERR "inside par\n";
            $insidepar = "Y";
        } 
        if ($newword =~ /\)[^(]*$/) {
            # print STDERR "outside par\n";
            $insidepar = "N";
        }
        ...
        # $period_quote_pos = rindex($newword, ".\"");
        # $period_2quote_pos = rindex($newword, ".\"\"");
        #$semicolon_pos = rindex($newword, ";"); # bourlon add
        ...
        if ($pos != -1)
        {
            ...
            if ($insidepar eq "Y" && $candidate eq ";") {
                $prediction = "N";
            } else {
...
sub Boundary
{
    ...
    # bourlon [add] debug info
    # print STDERR"candidate=$candidate wm2=$wm2 wm1=$wm1 prefix=$prefix suffix=$suffix wp1=$wp1 wp2=$wp2\n";
    ...
    #     # if ($suffix eq "sp" && $wp1let eq "Y" && IsHonorific($prefix."."))
    #     # {
    #     #    return "N";
    #     # }
    #     # # Check for the case when a capitalized word follows a period,
    #     # # and the prefix is a honorific
    #     # if ($suffix eq "sp" && $wp1let eq "Y" && StartsWithQuote($prefix))
    #     # {
    #     #    return "N";
    #     # }
    #     # This rule checks for prefixes that are terminal abbreviations
    #     if ($suffix eq "sp" && $wp1let eq "Y" && IsTerminal($prefix))
    #     {
    #         return "Y";
    #     }
    #     # # Check for the case when a capitalized word follows a period and the
    #     # # prefix is a single capital letter
    #     # if ($suffix eq "sp" && $wp1let eq "Y" && $prefix =~ /^([A-Z]\.)*[A-Z]$/)
    #     # {
    #     #    return "N";
    #     # }
    #     # Check for the case when a capitalized word follows a period
    ...
    else
    {
        ...
        # bourlon [add] split on ". (many occurences in "science opinion" data). broken: delete
        # if ($suffix =~ /\"+/) 
        # {
        #     print STDERR "suffix:$suffix wp1:$wp1 prefix:$prefix\n";
        #     print STDERR "case1\n";
        #     return "Y";
        # }
        if ($suffix eq "sp" && StartsWithQuote($wp1))
        ...
        # bourlon [add] this case to split on [community. Dr. William]
        if ($suffix eq "sp" && $wp1C eq "Y" && IsHonorific($wp1))
        {
            # print STDERR"case102\n";
            return "Y";
        }
        # Check for the case when a capitalized word follows a period,
        # and the prefix is a honorific
        ...
        # bourlon [del] comments and code do not match. 
        # This prevents from splitting on ["Community". This is] so comment out 
        # # Check for the case when a capitalized word follows a period,
        # # and the prefix is a honorific
        # if ($suffix eq "sp" && $wp1C eq "Y" && StartsWithQuote($prefix))
        # {
        #     # print STDERR"case111\n";
        #     return "N";
        # }
        ...
        # bourlon [del] many sentences on the pattern "group A. This..."
        # # Check for the case when a capitalized word follows a period and the
        # # prefix is a single capital letter
        ...
        # }
        # bourlon [add]
        if ($suffix eq "sp" && $wp1C eq "Y" && $wm1 =~ /\.$/)
        {
            # print STDERR"case13\n";
            return "N";
        }
        if ($suffix eq "sp" && $wp1C eq "Y" && $wp1 =~ /\.$/)
        {
            # print STDERR"case14\n";
            return "N";
        }
...
sub IsRightParen 
{
    ...
    # bourlon [add] "]"
    if ($word eq "}" ||  $word eq ")" || $word eq "]" || $word eq "-RBR-")
    {
        return 1;         # 1 means true
    }
    else
